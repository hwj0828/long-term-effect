...
gen.dat2 <- function(n1, n2, p){
  ...
  # ---------------  OBS  ---------------- # 
  X <- matrix(rnorm(n2*p, mean = 0, sd = 2), nrow = n2)   # X
  U <- rnorm(n2)                        # U, unmeasured confounder 
  ...
  # S <- Tr +  (X^2 %*% beta.s) + U^2  + rnorm(n2) # S
...
gen.dat5 <- function(n1, n2, p){
  ...
  beta.y = rep(1, p)
  prob.y = 1/(1 + exp(-Tr- 3*(X%*%beta.y) - U - S))
  Y <- rbinom(n1, 1, prob = prob.y)   # Y
...
gen.dat7 <- function(n1, n2, p){
  ...
  # ---------------  OBS  ---------------- # 
  X <- matrix(rbinom(n2*p, 1, prob = 0.3), nrow = n2)   # X
  U <- rnorm(n2)    # U, unmeasured confounder 
