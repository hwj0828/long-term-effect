  ...
  for(j in 1:3){
    ...
    for(loop in 1:1000){
      ...
      for(b in 1:B){
        ...
        res <- Est.con(dat.rct.tmp, dat.obs.tmp)
        ate.tmp[b] <- res$ate
      }
      ese.boot <- sd(ate.tmp)
    ...
    bias <- mean(PARA, na.rm = T) - ate0
    std <- sd(PARA, na.rm = T)
    ese <- mean(ESE, na.rm = T)
    CP95 <- mean(COUNT95, na.rm = T)
    ese.boot <- mean(ESE.boot,  na.rm = T)
    CP95.boot <- mean(COUNT95.boot,  na.rm = T)
