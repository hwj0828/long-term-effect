...
Est.DR <- function(dat.rct, dat.obs){
  ...
  mod.mu1 <- lm(Y ~. -Tr, data = dat.obs, subset = Tr == 1)
  mod.mu0 <- lm(Y ~. -Tr, data = dat.obs, subset = Tr == 0)
  
  mu1.obs <- predict.lm(mod.mu1, newdata = dat.obs)
  mu1.rct <- predict.lm(mod.mu1, newdata = dat.rct)
  mu0.obs <- predict.lm(mod.mu0, newdata = dat.obs)
  mu0.rct <- predict.lm(mod.mu0, newdata = dat.rct)
