...
Est.con <- function(dat.rct, dat.obs){
  ...
  # ----- estimate h function: binary outcome  ------ # 
  mod.ho <- lm(Y ~. , data = dat.obs)
  h.obs <- unname(mod.ho$fitted.values)
  h.rct <- predict.lm(mod.ho, newdata = dat.rct)
  ...
  I_a <-  t(X.obs) %*% X.obs / n2
  B <-  t((Tr.rct - ps.rct)/(ps.rct*(1-ps.rct))) %*%
    X.rct / n1
...
Est.con.reduce <- function(dat.rct, dat.obs){
